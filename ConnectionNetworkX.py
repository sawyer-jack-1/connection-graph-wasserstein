...
class ConnectionNetworkX(nx.DiGraph):
    ...
    def __init__(self, a, dim):
        ...
        self.connectionIncidenceMatrix = scipy.sparse.csr_matrix((self.nNodes * self.dimConnection, self.nEdges *
                                                                  self.dimConnection), dtype=float)
        self.connectionLaplacianMatrix = scipy.sparse.csr_matrix((self.nNodes * self.dimConnection, self.nNodes *
                                                                  self.dimConnection), dtype=float)
    ...
    def initializeConenctionLaplacian(self):
        ...
        for edgeIndex, e in zip(range(self.nEdges), self.edges()):
            ...
            for i in range(self.dimConnection):
                self.connectionIncidenceMatrix[fromNode * self.dimConnection + i, edgeIndex * self.dimConnection + i] = 1
                self.connectionIncidenceMatrix[toNode * self.dimConnection + i, edgeIndex * self.dimConnection + i] = -1

        self.connectionLaplacianMatrix = self.connectionIncidenceMatrix * self.connectionIncidenceMatrix.transpose()

        # print(self.connectionLaplacianMatrix.toarray())
    def updateEdgeSignature(self, edge, O):
        ...
        for i in range(self.dimConnection):
            for j in range(self.dimConnection):
                print(toNode * self.dimConnection + i, edgeIndex * self.dimConnection + j)
